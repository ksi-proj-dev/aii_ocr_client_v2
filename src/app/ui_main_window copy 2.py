...
class MainWindow(QMainWindow):
    ...
    def _handle_api_profile_selection(self):
        ...
        if selected_profile_id_from_cli:
            ...
            if profile_exists:
                ...
                self.log_manager.warning(f"コマンドライン指定APIプロファイルID '{selected_profile_id_from_cli}' は無効。", context="SYSTEM_INIT")
        if len(available_profiles) == 1:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted and dialog.selected_profile_id:
                ...
                self.log_manager.info(f"ユーザー選択APIプロファイル: '{self.active_api_profile.get('name')}'", context="SYSTEM_INIT")
            else: 
                self.log_manager.warning("APIプロファイル選択キャンセルまたは失敗。保存設定を使用。", context="SYSTEM_INIT")
                ...
                if not self.active_api_profile and available_profiles:
                     ...
                     self.config["current_api_profile_id"] = self.active_api_profile.get("id")
                     ConfigManager.save(self.config)
                     self.log_manager.info(f"フォールバックAPIプロファイル: '{self.active_api_profile.get('name')}'", context="SYSTEM_INIT")
    ...
    def _initialize_core_components_based_on_profile(self):
        ...
        if not self.active_api_profile:
            self.log_manager.critical("アクティブAPIプロファイル未設定でコンポーネント初期化不可。", context="MAINWIN_LIFECYCLE")
            return
    ...
    def append_log_message_to_widget(self, level: str, message: str):
        if hasattr(self, 'log_widget') and self.log_widget:
            ...
            if color:
                self.log_widget.append(f'<font color="{color}">{escaped_message}</font>')
            else:
                self.log_widget.append(escaped_message)
    ...
    def select_input_folder(self):
        ...
        if folder:
            ...
            if folder != self.input_folder_path or not self.processed_files_info:
                self.input_folder_path = folder; self._update_folder_display()
                self.log_manager.info(f"Performing rescan for newly selected folder: {folder}", context="UI_EVENT"); self.perform_rescan()
    ...
    def _handle_ocr_process_finished_from_orchestrator(self, was_interrupted: bool, fatal_error_info: Optional[Dict[str, Any]] = None):
    ...
    def perform_initial_scan(self):
        self.log_manager.info(f"スキャン開始: {self.input_folder_path}", context="FILE_SCAN_MAIN"); self.update_timer.stop() if self.update_timer.isActive() else None; self.processed_files_info = []
        paths, max_info, depth_info = self.file_scanner.scan_folder(self.input_folder_path)
        if paths: self.processed_files_info = self.file_scanner.create_initial_file_list(paths, OCR_STATUS_SKIPPED_SIZE_LIMIT, OCR_STATUS_NOT_PROCESSED); self.log_manager.info(f"スキャン完了: {len(paths)}件読込 ({sum(1 for i in self.processed_files_info if i.ocr_engine_status != OCR_STATUS_SKIPPED_SIZE_LIMIT)}件処理対象)", context="FILE_SCAN_MAIN")
        else: self.log_manager.info("スキャン完了: 対象ファイル0件", context="FILE_SCAN_MAIN")
        if hasattr(self, 'list_view'): self.list_view.update_files(self.processed_files_info)
        warnings = [f"最大処理ファイル数 ({max_info['limit']}件)到達のため「{max_info['last_scanned_folder']}」以降一部未読込。" if max_info else "", f"再帰検索深度制限により「{', '.join([os.path.basename(f) for f in depth_info[:3]])}{f'等計{len(depth_info)}フォルダ' if len(depth_info)>3 else ''}」以降未検索。" if depth_info else ""]
        if any(warnings): QMessageBox.warning(self, "スキャン結果注意", "\n\n".join(filter(None, warnings)))
    ...
    def open_input_folder_in_explorer(self):
        ...
        if self.input_folder_path and os.path.isdir(self.input_folder_path):
            ...
            except Exception as e: self.log_manager.error(f"Failed to open folder '{self.input_folder_path}'. Error: {e}", context="UI_ACTION_OPEN_FOLDER_ERROR", exception_info=e); QMessageBox.warning(self, "フォルダを開けません", f"フォルダ '{self.input_folder_path}' を開けませんでした。\nエラー: {e}")
    ...
    def toggle_view(self):
        if hasattr(self, 'stack'): self.current_view = 1 - self.current_view; self.stack.setCurrentIndex(self.current_view); self.log_manager.info(f"View toggled to: {'ListView' if self.current_view == 1 else 'SummaryView'}", context="UI_ACTION")
    ...
    def show_option_dialog(self):
        ...
        old_opts = current_option_values.copy() if current_option_values else {} 
        old_max_files = old_opts.get("max_files_to_process"); old_recursion_depth = old_opts.get("recursion_depth")

        dialog = OptionDialog(options_schema, old_opts, self.config, self)
        if dialog.exec():
            updated_profile_opts, updated_global_cfg = dialog.get_saved_settings()
            if active_profile_id: self.config["options_values_by_profile"][active_profile_id] = updated_profile_opts
            for k, v in updated_global_cfg.items(): 
                if k not in ["api_profiles", "current_api_profile_id", "options_values_by_profile"]: self.config[k] = v 
            ConfigManager.save(self.config); self.log_manager.info("Options saved and reloaded into MainWindow.", context="CONFIG_EVENT")
            self.active_api_profile = ConfigManager.get_active_api_profile(self.config) 
            if self.api_client: self.api_client.update_config(self.config, self.active_api_profile)
            if self.ocr_orchestrator: self.ocr_orchestrator.update_config(self.config, self.active_api_profile)
            if self.file_scanner: self.file_scanner.config = self.config
            self._update_window_title(); self._update_api_mode_toggle_button_display()
            
            new_opts = ConfigManager.get_active_api_options_values(self.config) or {}
            if old_max_files != new_opts.get("max_files_to_process") or old_recursion_depth != new_opts.get("recursion_depth"):
                if self.input_folder_path and os.path.isdir(self.input_folder_path): QMessageBox.information(self, "設定変更", "ファイル検索設定変更のため再スキャンします。"); self.perform_rescan()
                else: self.processed_files_info = []; self.list_view.update_files([]) if hasattr(self, 'list_view') else None; self.update_all_status_displays(); self.log_manager.info("検索設定変更、入力フォルダなし、リストクリア。", context="CONFIG_EVENT")
            else: 
                self.log_manager.info(f"Settings changed. Re-evaluating file statuses based on new options.", context="CONFIG_EVENT")
                new_upload_max_mb = new_opts.get("upload_max_size_mb", 60); new_upload_max_bytes = new_upload_max_mb * 1024 * 1024
                new_file_actions = self.config.get("file_actions", {}); new_output_format = new_file_actions.get("output_format", "both")
                default_json = "-" if new_output_format in ["json_only", "both"] else "作成しない(設定)"; default_pdf = "-" if new_output_format in ["pdf_only", "both"] else "作成しない(設定)"
                items_updated = False
                for fi in self.processed_files_info:
                    prev_engine_status = fi.ocr_engine_status; prev_checked = fi.is_checked; orig_status = fi.status; orig_json = fi.json_status; orig_pdf = fi.searchable_pdf_status
                    is_now_skipped = fi.size > new_upload_max_bytes
                    if is_now_skipped:
                        if fi.ocr_engine_status != OCR_STATUS_SKIPPED_SIZE_LIMIT: fi.status = "スキップ(サイズ上限)"; fi.ocr_engine_status = OCR_STATUS_SKIPPED_SIZE_LIMIT; fi.ocr_result_summary = f"ファイルサイズが上限 ({new_upload_max_mb}MB) を超過"; fi.json_status = "スキップ"; fi.searchable_pdf_status = "スキップ"; fi.is_checked = False
                    else:
                        if fi.ocr_engine_status == OCR_STATUS_SKIPPED_SIZE_LIMIT: fi.status = "待機中"; fi.ocr_engine_status = OCR_STATUS_NOT_PROCESSED; fi.ocr_result_summary = ""; fi.is_checked = True
                        if fi.ocr_engine_status == OCR_STATUS_NOT_PROCESSED: fi.json_status = default_json; fi.searchable_pdf_status = default_pdf
                    if (fi.ocr_engine_status != prev_engine_status or fi.status != orig_status or fi.json_status != orig_json or fi.searchable_pdf_status != orig_pdf or fi.is_checked != prev_checked): items_updated = True
    ...
    def confirm_resume_ocr(self):
        if hasattr(self, 'ocr_orchestrator'): self.ocr_orchestrator.confirm_and_resume_ocr(self.processed_files_info, self.input_folder_path, self)
    ...
    def on_file_ocr_processed(self, original_file_main_idx: int, original_file_path: str, 
                              ocr_result_data_for_original: Any, 
                              ocr_error_info_for_original: Optional[Dict[str, Any]], 
                              json_save_status_for_original: Any):
        ...
        if ocr_error_info_for_original and isinstance(ocr_error_info_for_original, dict):
            ...
            target_file_info.ocr_result_summary = f"エラー ({err_code})" if err_code else "エラー (詳細はログ参照)"
            self.log_manager.error(f"OCR処理エラー詳細 (ファイル: {target_file_info.name}): Msg='{err_msg}', Code='{err_code}', Detail='{err_detail}'", context="OCR_PROCESS_ERROR_DETAIL")

            if err_code not in ["USER_INTERRUPT", "NOT_IMPLEMENTED_LIVE_API", "NOT_IMPLEMENTED_API_CALL", "FATAL_ERROR_STOP", "PART_PROCESSING_ERROR"]:
        ...
        if not processed_as_error and ocr_result_data_for_original:
            ...
            if isinstance(ocr_result_data_for_original, dict) and ocr_result_data_for_original.get("status") == "parts_processed_ok":
                target_file_info.ocr_result_summary = "成功 (分割処理)"
                detail_message = ocr_result_data_for_original.get('detail', '')
                self.log_manager.info(f"ファイル '{target_file_info.name}' のOCR処理(分割): {detail_message}", context="OCR_PROCESS_DETAIL")
            else: 
                extracted_fulltext = ""
                if isinstance(ocr_result_data_for_original, list) and ocr_result_data_for_original:
                    try:
                        first_page_content = ocr_result_data_for_original[0]
                        result_field = first_page_content.get("result", {})
                        fulltext_parts = []
                        if "fulltext" in result_field and result_field["fulltext"]: fulltext_parts.append(result_field["fulltext"])
                        if "aGroupingFulltext" in result_field and result_field["aGroupingFulltext"]: fulltext_parts.append(result_field["aGroupingFulltext"])
                        extracted_fulltext = " / ".join(filter(None, fulltext_parts))
                    except Exception as e:
                        extracted_fulltext = ""; self.log_manager.error(f"OCR結果(リスト)の解析中にエラー: {e} (データ: {ocr_result_data_for_original})", context="UI_OCR_RESULT_PARSE", exc_info=True)
                    ...
                    if results_list and isinstance(results_list, list) and results_list[0].get("pages"):
                        first_page = results_list[0]["pages"][0] if results_list[0]["pages"] else {}
                        extracted_fulltext = first_page.get("fulltext", "")
                    elif "fulltext" in ocr_result_data_for_original: 
                        extracted_fulltext = ocr_result_data_for_original.get("fulltext", "")
                    else:
                        self.log_manager.warning(f"OCR結果(辞書)の形式が予期したものではありません: {ocr_result_data_for_original}", context="UI_OCR_RESULT_PARSE")
                else:
                    self.log_manager.warning(f"OCR結果の形式がリストでも辞書でもありません: {type(ocr_result_data_for_original)}", context="UI_OCR_RESULT_PARSE")

                fulltext_to_log = extracted_fulltext 
                if extracted_fulltext:
                    target_file_info.ocr_result_summary = "成功 (テキストあり)"
                    log_text = (fulltext_to_log[:200] + '...') if len(fulltext_to_log) > 200 else fulltext_to_log
                    self.log_manager.debug(f"ファイル '{target_file_info.name}' の抽出テキスト(一部): {log_text}", context="OCR_EXTRACTED_TEXT_DETAIL")
                else:
                    target_file_info.ocr_result_summary = "成功 (テキストなし)"
        
        if not processed_as_error and not ocr_result_data_for_original:
            target_file_info.status = "OCR状態不明"
            target_file_info.ocr_engine_status = OCR_STATUS_FAILED 
            target_file_info.ocr_result_summary = "応答なし(OCR)" 
    ...
    def on_file_searchable_pdf_processed(self, original_file_main_idx: int, original_file_path: str, 
                                     pdf_final_path: Optional[str], 
                                     pdf_error_info: Optional[Dict[str, Any]]):
