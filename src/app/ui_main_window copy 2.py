...
class MainWindow(QMainWindow):
    ...
    def _initialize_core_components_based_on_profile(self):
        ...
        # self.log_manager は __init__ の最初で初期化済み
    ...
    def _connect_orchestrator_signals(self):
        if hasattr(self, 'ocr_orchestrator') and self.ocr_orchestrator:
            ...
            self.ocr_orchestrator.request_list_view_update_signal.connect(self._handle_request_list_view_update) # ★ ここで接続

    # ★★★ Missing method definition from traceback ★★★
    def _handle_request_list_view_update(self, updated_file_list: List[FileInfo]):
        """OcrOrchestratorからのListView更新要求を処理する"""
        self.log_manager.debug("MainWindow: Received request to update ListView from orchestrator.", context="UI_UPDATE")
        self.processed_files_info = updated_file_list # メインのファイル情報リストを更新
        if hasattr(self, 'list_view') and self.list_view:
            self.list_view.update_files(self.processed_files_info) # ListViewウィジェットを更新
        self.update_all_status_displays() # サマリービューやステータスバーも更新
    ...
    def _setup_toolbars_and_folder_labels(self):
        ...
        # self._update_api_mode_toggle_button_display() # _load_previous_state...で呼ばれるのでここでは不要かも
    ...
    def _load_previous_state_and_perform_initial_scan(self):
        ...
        if self.input_folder_path and os.path.isdir(self.input_folder_path):
            ...
            self.input_folder_path = ""; self._update_folder_display(); self._clear_and_update_file_list_display()
        else:
            self.log_manager.info("前回終了時の入力フォルダ指定はありませんでした。", context="SYSTEM_INIT")
            self._update_folder_display(); self._clear_and_update_file_list_display()
    ...
    def _clear_and_update_file_list_display(self):
        ...
        if hasattr(self, 'summary_view') and self.summary_view: self.summary_view.reset_summary()
    ...
    def _handle_ocr_process_started_from_orchestrator(self, num_files_to_process: int, updated_file_list: List[FileInfo]):
        ...
        self.is_ocr_running = True; self.processed_files_info = updated_file_list
        if hasattr(self, 'list_view') and self.list_view: self.list_view.update_files(self.processed_files_info)
    ...
    def _handle_ocr_process_finished_from_orchestrator(self, was_interrupted: bool, fatal_error_info: Optional[dict] = None):
        ...
        self.is_ocr_running = False; reason = ""
        ...
        if was_interrupted or fatal_error_info:
            ...
            json_interrupt_status = "中断" if output_format_cfg in ["json_only", "both"] else "作成しない(設定)"; pdf_interrupt_status = "中断" if output_format_cfg in ["pdf_only", "both"] else "作成しない(設定)"
            for file_info in self.processed_files_info:
                if file_info.ocr_engine_status in [OCR_STATUS_PROCESSING, OCR_STATUS_SPLITTING, OCR_STATUS_PART_PROCESSING, OCR_STATUS_MERGING] or \
                   (file_info.status == OCR_STATUS_PROCESSING and file_info.ocr_engine_status == OCR_STATUS_PROCESSING):
                    file_info.ocr_engine_status = OCR_STATUS_FAILED; file_info.status = "中断" if was_interrupted else "エラー(停止)"; file_info.ocr_result_summary = f"(処理が中止/停止されました: {reason})" if reason else "(処理が中止/停止されました)"
                    file_info.json_status = json_interrupt_status; file_info.searchable_pdf_status = pdf_interrupt_status
        ...
        elif was_interrupted: final_message = "OCR処理が中止されました。"; QMessageBox.information(self, "処理終了", final_message)
        else: QMessageBox.information(self, "処理終了", final_message)

    def update_status_bar(self):
        total_list_items = len(self.processed_files_info); selected_for_processing_count = 0
        ...
        if hasattr(self, 'status_total_list_label'):
            self.status_total_list_label.setText(f"リスト総数: {total_list_items}"); self.status_selected_files_label.setText(f"選択中: {selected_for_processing_count}")
            self.status_success_files_label.setText(f"成功: {ocr_success_count}"); self.status_error_files_label.setText(f"エラー: {ocr_error_count}")
    ...
    def perform_initial_scan(self):
        ...
        if depth_limited_folders:
            folders_to_show = depth_limited_folders[:3]; folders_str = ", ".join([os.path.basename(f) for f in folders_to_show])
            if len(depth_limited_folders) > 3: folders_str += f" など、計{len(depth_limited_folders)}フォルダ"
            msg = f"再帰検索の深さ制限により、サブフォルダ「{folders_str}」以降は検索されていません。"
            warning_messages.append(msg)
        if warning_messages: QMessageBox.warning(self, "スキャン結果の注意", "\n\n".join(warning_messages))
    ...
    def append_log_message_to_widget(self, level, message):
        if hasattr(self, 'log_widget') and self.log_widget:
            color_map = {LogLevel.ERROR: "red", LogLevel.WARNING: "orange", LogLevel.DEBUG: "gray"}
            color = color_map.get(level)
            if color: self.log_widget.append(f'<font color="{color}">{message}</font>')
    ...
    def on_file_ocr_processed(self, original_file_main_idx, original_file_path, ocr_result_data_for_original, ocr_error_info_for_original, json_save_status_for_original):
        ...
        if ocr_error_info_for_original and isinstance(ocr_error_info_for_original, dict):
            ...
            if err_code not in ["USER_INTERRUPT", "NOT_IMPLEMENTED_LIVE_API", "NOT_IMPLEMENTED_API_CALL", "FATAL_ERROR_STOP"]: QMessageBox.warning(self, f"OCR処理エラー ({target_file_info.name})", f"ファイル「{target_file_info.name}」のOCR処理中にエラーが発生しました。\n\nメッセージ: {err_msg}\nコード: {err_code}\n詳細: {err_detail if err_detail else 'N/A'}\n\nログファイルをご確認ください。")
            ...
            if isinstance(ocr_result_data_for_original, dict):
                if "detail" in ocr_result_data_for_original: fulltext = ocr_result_data_for_original["detail"]
    ...
    def on_file_searchable_pdf_processed(self, original_file_main_idx, original_file_path, pdf_final_path, pdf_error_info):
        ...
        if output_format == "json_only": target_file_info.searchable_pdf_status = "作成しない(設定)"
        elif pdf_final_path and not pdf_error_info and os.path.exists(pdf_final_path): target_file_info.searchable_pdf_status = "PDF作成成功"; pdf_stage_final_success = True; target_file_info.status = "完了" if ocr_engine_status == OCR_STATUS_COMPLETED else target_file_info.status
            ...
            if error_code == "PARTS_COPIED_SUCCESS": pdf_stage_final_success = True; target_file_info.status = "完了" if ocr_engine_status == OCR_STATUS_COMPLETED else target_file_info.status
            elif error_code in ["PARTS_COPIED_PARTIAL", "PARTS_COPY_ERROR", "NO_PARTS_TO_COPY"] and ocr_engine_status == OCR_STATUS_COMPLETED: target_file_info.status = "部品PDFエラー"
            elif not ("作成対象外" in error_msg or "作成しない" in error_msg or "部品PDFは結合されません(設定)" in error_msg):
                target_file_info.searchable_pdf_status = "PDF作成失敗"
                if ocr_engine_status == OCR_STATUS_COMPLETED: target_file_info.status = "PDF作成失敗"; target_file_info.ocr_result_summary = (target_file_info.ocr_result_summary or "") + f" (PDFエラー: {error_msg})" if "部品のOCR完了" not in (target_file_info.ocr_result_summary or "") and "PDFエラー" not in (target_file_info.ocr_result_summary or "") else (f"PDFエラー: {error_msg}" if "部品のOCR完了" not in (target_file_info.ocr_result_summary or "") else target_file_info.ocr_result_summary)
            if error_code not in ["USER_INTERRUPT_PDF", "PDF_NOT_REQUESTED", "PARTS_COPIED_SUCCESS", "NO_PARTS_TO_COPY", "PDF_CREATION_FAIL_DUE_TO_OCR_ERROR", "NOT_IMPLEMENTED_LIVE_API_PDF", "NOT_IMPLEMENTED_API_CALL_PDF", "FATAL_ERROR_STOP_PDF"]: QMessageBox.warning(self, f"PDF処理エラー ({target_file_info.name})", f"ファイル「{target_file_info.name}」のPDF処理エラー。\n\nメッセージ: {error_msg}\nコード: {error_code}\n詳細: {err_detail or 'N/A'}\n\nログ参照。")
        elif ocr_engine_status == OCR_STATUS_FAILED: target_file_info.searchable_pdf_status = "対象外(OCR失敗)"
        elif output_format in ["pdf_only", "both"]: target_file_info.searchable_pdf_status = "PDF状態不明"; target_file_info.status = "PDF状態不明" if ocr_engine_status == OCR_STATUS_COMPLETED else target_file_info.status
