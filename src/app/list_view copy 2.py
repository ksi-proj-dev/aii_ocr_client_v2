...
class ListView(QWidget):
    ...
    def toggle_all_checkboxes(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if item and item.flags() & Qt.ItemFlag.ItemIsUserCheckable and item.flags() & Qt.ItemFlag.ItemIsEnabled:
                ...
                # Note: 全選択トグルでは個別のitem_check_state_changedシグナルはemitしない方が効率的かもしれない
                #       必要であれば、最後に一括更新シグナルをemitする
        self._suspend_item_changed_signal = False
        # 全選択後、一括で更新が必要な場合は、ここで専用のシグナルをemitするか、MainWindowから直接状態を更新する
        # 例えば、最初のチェックボックスの変更シグナルだけを手動でemitするなど
    ...
    def on_item_changed(self, item: QTableWidgetItem):
        ...
        if item.column() == 0: # チェックボックス列
            row = item.row()
            # 表示上の行インデックスから、ソート状態を考慮した元のデータインデックスを見つける必要があるが、
            # Noをキーにするのが確実。
