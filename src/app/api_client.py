...
import json
import random
import shutil # shutil は OcrWorker で使用
# import requests # 実際のAPIコール時に必要
from log_manager import LogLevel # LogManagerからLogLevelをインポート
...
class CubeApiClient:
    ...
    def read_document(self, file_path):
        ...
        # APIに渡す実際のOCRオプションを構築 (API仕様のパラメータ名に合わせる)
        actual_ocr_params = {
        ...
        # Noneの値を持つキーはAPIに送信しない (APIの仕様による)
        ...
        if self.dummy_mode:
            ...
            page_result = {
                ...
                "result": {
                    ...
                    "results": [{"text": "ダミーテキスト1", "bbox": {}, "ocrConfidence": 0.95, "detectConfidence": 0.99, "vertices": [], "characters": []}],
                    "tables": [], "textGroups": []
                }, "status": "success"
            }
            if actual_ocr_params.get('character_extraction') == 1:
                if page_result["result"]["results"]: # resultsが空でないことを確認
                    for res_item in page_result["result"]["results"]:
                        if res_item.get("text"): # textキーが存在することを確認
            ...
            if actual_ocr_params.get('fulltext') == 1: # fulltextモードの場合
                simplified_result = {
            ...
            self.log_manager.info(f"  ヘッダーキー: {list(headers.keys())}", context=log_ctx) # APIキー自体はログに出さない
    ...
    def make_searchable_pdf(self, file_path):
        ...
        if self.dummy_mode:
            ...
            self.log_manager.info(f"'/make-searchable-pdf' ダミー呼び出し完了: {file_name}", context=log_ctx)
            ...
            try:
                with open(file_path, 'rb') as f:
                    ...
                    # self.log_manager.info(f"  API応答ステータス: {response.status_code}", context=log_ctx, filename=file_name)
                    # if not response.ok:
                    #     self.log_manager.error(f"  APIエラーレスポンス (ステータス {response.status_code})", context=log_ctx, filename=file_name, response_text=response.text[:200]) # PDFエラーはテキストでない可能性
                    # response.raise_for_status()
