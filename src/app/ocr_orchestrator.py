...
from log_manager import LogManager, LogLevel # LogLevel をインポート
...
class OcrOrchestrator(QObject):
    ...
    def _prepare_and_start_ocr_worker(self, files_to_send_to_worker_tuples, input_folder_path):
        ...
        if not self.api_client:
            self.log_manager.error("OcrOrchestrator: ApiClient is not configured. Cannot start OCR worker.", context="OCR_ORCH_ERROR", error_code="NO_API_CLIENT") # ★ error_code 追加
            self.is_ocr_running = False
    ...
    def _handle_worker_all_files_processed(self):
        ...
        if self.ocr_worker:
            was_interrupted_by_user = self.ocr_worker.user_stopped
            try: self.ocr_worker.original_file_status_update.disconnect(self._handle_worker_status_update)
            except (TypeError, RuntimeError): pass
            try: self.ocr_worker.file_processed.disconnect(self._handle_worker_file_ocr_processed)
            except (TypeError, RuntimeError): pass
            try: self.ocr_worker.searchable_pdf_processed.disconnect(self._handle_worker_searchable_pdf_processed)
            except (TypeError, RuntimeError): pass
            try: self.ocr_worker.all_files_processed.disconnect(self._handle_worker_all_files_processed)
            except (TypeError, RuntimeError): pass
            self.ocr_worker = None
    ...
    def confirm_and_stop_ocr(self, parent_widget_for_dialog):
        ...
        if worker_to_stop is not None and hasattr(worker_to_stop, 'isRunning') and worker_to_stop.isRunning():
            ...
            if self.is_ocr_running:
                ...
                self.is_ocr_running = False
                self.ocr_process_finished_signal.emit(True, self.fatal_error_occurred_info)
            self.request_ui_controls_update_signal.emit()
    ...
    def update_config(self, new_config: dict):
        ...
        if self.api_client:
            self.api_client.update_config(new_config)