...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QMessageBox, QGroupBox, QSpinBox, QRadioButton, # ★ QRadioButton をインポートリストに追加
    QVBoxLayout, QLabel,
    QWidget 
)
# from config_manager import ConfigManager # 直接は使用しない
...
class OptionDialog(QDialog):
    ...
    def init_ui(self):
        ...
        api_key_group = QGroupBox("APIキー設定")
        api_key_form_layout = QFormLayout()
        self.api_key_edit = QLineEdit(self.global_config.get("api_key", ""))
        self.api_key_edit.setPlaceholderText("APIキーを入力 (Liveモードで必須)")
        api_key_form_layout.addRow("共通APIキー:", self.api_key_edit)
    ...
    def on_save_settings(self):
        ...
        updated_global_config = self.global_config.copy() 
        updated_global_config["api_key"] = self.api_key_edit.text().strip()
